...
<div className="mt-1 flex items-center justify-between px-1 sm:hidden"><button type="button" onClick={() => { const el = document.querySelector("[data-exercise-carousel]"); el?.scrollBy({ left: -(el.clientWidth + 16), behavior: "smooth" }); }} className="flex size-8 items-center justify-center rounded-full border border-[#e6d8c5] bg-white text-[#A97A3C] shadow-[0_5px_14px_rgba(64,48,30,0.10)]" aria-label="Exercício anterior">‹</button><div className="flex items-center gap-1.5">{paginatedExercises.map((exercise, index) => <button key={exercise.id} type="button" onClick={() => { const el = document.querySelector("[data-exercise-carousel]"); const card = el?.querySelector("[data-exercise-card]"); el?.scrollTo({ left: index * ((card?.clientWidth ?? 0) + 16), behavior: "smooth" }); }} className={`h-1.5 rounded-full transition-all ${index === mobileSlide ? "w-4 bg-[#A97A3C]" : "w-1.5 bg-[#d9c8b2]"}`} aria-label={`Ir para o exercício ${index + 1}`} />)}</div><button type="button" onClick={() => { const el = document.querySelector("[data-exercise-carousel]"); el?.scrollBy({ left: el.clientWidth + 16, behavior: "smooth" }); }} className="flex size-8 items-center justify-center rounded-full border border-[#e6d8c5] bg-white text-[#A97A3C] shadow-[0_5px_14px_rgba(64,48,30,0.10)]" aria-label="Próximo exercício">›</button></div>
          </div>
        </div>
