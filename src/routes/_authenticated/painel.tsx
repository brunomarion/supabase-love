...
function Exercises({ exercises, onAdd, onEdit, onDelete, onView, deleting }: { exercises: Exercise[]; onAdd: () => void; onEdit: (exercise: Exercise) => void; onDelete: (exercise: Exercise) => void; onView: (exercise: Exercise) => void; deleting: string | null }) {
  ...
  useEffect(() => {
    setCarouselIndex(0);
  }, [search, filteredExercises.length]);

  const goToExercise = (index: number) => {
    const container = document.getElementById("exercises-carousel");
    const card = container?.querySelector<HTMLElement>("[data-exercise-card]");
    if (!container || !card) return;
    const gap = 16;
    const target = index * (card.offsetWidth + gap);
  ...
  const scrollCarousel = (direction: -1 | 1) => {
    ...
    const next = (carouselIndex + direction + filteredExercises.length) % filteredExercises.length;
    goToExercise(next);
  };

  return <section className="space-y-5"><Header title="Exercícios" text="Biblioteca de exercícios em vídeo." action="Adicionar exercício" onAction={onAdd} /><div className="relative w-full"><Search className="pointer-events-none absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-[#A97A3C]" /><input type="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Pesquisar exercício..." aria-label="Pesquisar exercícios" className="h-11 w-full rounded-xl border border-[#e6d8c5] bg-white pl-10 pr-10 text-base text-[#302b26] shadow-[0_6px_18px_rgba(64,48,30,0.04)] outline-none transition-all duration-200 placeholder:text-[#a59b92] focus:border-[#BA9051] focus:ring-2 focus:ring-[#BA9051]/10 sm:text-sm" />{search && <button type="button" onClick={() => setSearch("")} aria-label="Limpar pesquisa" className="absolute right-3 top-1/2 flex size-6 -translate-y-1/2 items-center justify-center rounded-full text-[#8b8178] transition hover:bg-[#f4ece1] hover:text-[#A97A3C]"><X className="size-4" /></button>}</div><div className="-mx-1 overflow-hidden sm:mx-0 sm:overflow-visible">{exercises.length === 0 ? <div className="sm:col-span-2 xl:col-span-3"><Empty text="Nenhum exercício cadastrado ainda." /></div> : filteredExercises.length === 0 ? <div className="rounded-[1.2rem] border border-dashed border-[#dccbb5] bg-white p-8 text-center text-xs text-[#8c8178]">Nenhum exercício encontrado para sua pesquisa.</div> : <><div className="relative px-8 sm:px-0"><button type="button" onClick={() => scrollCarousel(-1)} aria-label="Exercício anterior" className="absolute left-0 top-1/2 z-10 hidden size-9 -translate-y-1/2 items-center justify-center rounded-full border border-[#dfcdb3] bg-white/95 text-[#A97A3C] shadow-[0_8px_22px_rgba(64,48,30,0.14)] backdrop-blur-sm transition hover:-translate-y-1/2 hover:border-[#BA9051] hover:bg-[#fffaf2] sm:flex md:left-1"><span className="text-xl leading-none">‹</span></button><div id="exercises-carousel" onScroll={(e) => { const target = e.currentTarget; const card = target.querySelector<HTMLElement>("[data-exercise-card]"); if (!card) return; const next = Math.round(target.scrollLeft / (card.offsetWidth + 16)); setCarouselIndex(Math.max(0, Math.min(next, filteredExercises.length - 1))); }} className="flex gap-4 overflow-x-auto px-1 pb-3 snap-x snap-mandatory [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:grid sm:grid-cols-2 sm:gap-5 sm:overflow-visible sm:px-0 sm:pb-0 sm:snap-none xl:grid-cols-3">{filteredExercises.map((e) => <article data-exercise-card key={e.id} className="group w-[78vw] max-w-[270px] shrink-0 snap-center overflow-hidden rounded-[1.2rem] border border-[#e6d9c9] bg-white shadow-[0_10px_30px_rgba(64,48,30,0.055)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_18px_42px_rgba(64,48,30,0.11)] sm:w-auto sm:max-w-none sm:shrink sm:snap-none sm:rounded-[1.35rem]"><button type="button" onClick={() => onView(e)} className="relative block aspect-video w-full overflow-hidden bg-[linear-gradient(145deg,#f7eee2,#ead9bf)] text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-[#BA9051] focus-visible:ring-inset touch-manipulation">{e.thumbnail_url ? <img src={e.thumbnail_url} alt={"Capa do exercício " + e.name} className="size-full object-cover transition-transform duration-500 group-hover:scale-[1.035]" /> : <div className="flex size-full items-center justify-center"><Dumbbell className="size-12 text-[#BA9051]/45" /></div>}<div className="absolute inset-0 bg-gradient-to-t from-[#2d2823]/45 via-transparent to-transparent" /><span className="absolute left-3 top-3 rounded-full border border-white/30 bg-[#2d2823]/55 px-2 py-1 text-[8px] font-semibold uppercase tracking-[0.12em] text-white backdrop-blur-md sm:left-4 sm:top-4 sm:px-2.5 sm:py-1 sm:text-[9px]">{e.type || "Vídeo"}</span><span className="absolute left-1/2 top-1/2 flex size-11 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-white/70 bg-white/90 text-[#A97A3C] shadow-[0_10px_28px_rgba(45,40,35,0.25)] transition-transform duration-300 group-hover:scale-110 sm:size-12"><Play className="ml-0.5 size-5 fill-current" /></span></button><div className="p-4 sm:p-5"><h3 className="truncate text-sm font-semibold text-[#302b26]">{e.name}</h3><p className="mt-1.5 line-clamp-2 min-h-9 text-xs leading-relaxed text-[#81776e]">{e.description || "Exercício em vídeo."}</p><div className="mt-3 flex items-center justify-end gap-3 sm:mt-4"><div className="flex gap-2"><IconButton label="Editar" onClick={() => onEdit(e)}><Pencil className="size-4" /></IconButton><IconButton label="Excluir" onClick={() => onDelete(e)} disabled={deleting === e.id}>{deleting === e.id ? <RefreshCw className="size-4 animate-spin" /> : <Trash2 className="size-4" />}</IconButton></div></div></div></article>)}</div><button type="button" onClick={() => scrollCarousel(1)} aria-label="Próximo exercício" className="absolute right-0 top-1/2 z-10 hidden size-9 -translate-y-1/2 items-center justify-center rounded-full border border-[#dfcdb3] bg-white/95 text-[#A97A3C] shadow-[0_8px_22px_rgba(64,48,30,0.14)] backdrop-blur-sm transition hover:-translate-y-1/2 hover:border-[#BA9051] hover:bg-[#fffaf2] sm:flex md:right-1"><span className="text-xl leading-none">›</span></button></div><div className="flex items-center justify-center gap-1.5 pt-1 sm:hidden">{filteredExercises.map((exercise, index) => <button key={exercise.id} type="button" onClick={() => goToExercise(index)} aria-label={"Ir para " + exercise.name} className={`h-1.5 rounded-full transition-all duration-300 ${index === carouselIndex ? "w-6 bg-[#BA9051] shadow-[0_0_10px_rgba(186,144,81,0.28)]" : "w-1.5 bg-[#d8c8b5]"}`} />)}</div></>}</div>}</div></section>;
}
