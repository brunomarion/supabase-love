                ...
                <div className="max-h-[46vh] space-y-3 overflow-y-auto pr-1">
                  {loadingSessions ? (
                    <div className="flex min-h-[150px] items-center justify-center text-xs text-[#948a81]">
                      <RefreshCw className="mr-2 size-4 animate-spin text-[#BA9051]" /> Carregando atendimentos...
                    </div>
        ...
        {filteredPatients.length === 0 ? <Empty text={statusFilter === "all" ? "Nenhum paciente cadastrado ainda." : statusFilter === "active" ? "Nenhum paciente ativo encontrado." : "Nenhum paciente inativo encontrado."} /> : <>
          ...
          <div className="hidden sm:block">{paginatedPatients.map((p) => (
            <div key={p.id} className={`grid grid-cols-[minmax(0,1fr)_auto] gap-2.5 border-b border-[#d9c8b4] px-3 py-3 sm:grid-cols-[1.3fr_1.1fr_1fr_0.8fr_110px] sm:items-center sm:gap-4 sm:px-5 sm:py-4 ${p.sex === "female" ? "bg-[#fff1f6] hover:bg-[#ffebf2]" : p.sex === "male" ? "bg-[#eff7ff] hover:bg-[#e7f2ff]" : "bg-white hover:bg-[#fdfbf8]"} transition-colors`}>
              ...
              <div className="flex min-w-0 items-center gap-2 text-[14px] font-medium text-[#5f574f]">
                <span className="min-w-0">{visibleCpfs.has(p.id) ? formatCpf(p.cpf ?? "") || "Não informado" : getMaskedCpf(p.cpf)}</span>
...
function Exercises({ exercises, pdfMaterials, onAdd, onAddPdf, onEdit, onDelete, onEditPdf, onDeletePdf, onView, deleting }: { exercises: Exercise[]; pdfMaterials: PdfMaterial[]; onAdd: () => void; onAddPdf: () => void; onEdit: (exercise: Exercise) => void; onDelete: (exercise: Exercise) => void; onEditPdf: (pdf: PdfMaterial) => void; onDeletePdf: (pdf: PdfMaterial) => void; onView: (exercise: Exercise) => void; deleting: string | null }) {
  ...
  return <section className="space-y-5">
        ...
        <div className="flex items-center justify-end gap-3 lg:gap-4">
          <div className="hidden items-center gap-3 lg:flex">
            <div className="flex items-center gap-2 rounded-xl border border-[#dfc28f] bg-white/95 px-3 py-2 shadow-[0_6px_20px_rgba(186,144,81,0.12)]">
              <span className="text-[18px] font-semibold tracking-[-0.03em] text-[#BA9051]">{contentType === "pdfs" ? pdfMaterials.length : exercises.length}</span>
              <span className="text-[9px] font-semibold uppercase tracking-[0.08em] text-[#948a81]">{contentType === "pdfs" ? "materiais cadastrados" : "exercícios cadastrados"}</span>
            </div>
    ...
    <div className="hidden lg:block relative rounded-2xl border border-[#e6d9c9] bg-white p-2 shadow-[0_6px_20px_rgba(64,48,30,0.045)]">
      <label className="relative flex h-11 items-center gap-2.5 rounded-xl border border-[#e6d9c9] bg-[#fdfbf8] px-3 pr-12 text-[#837970] lg:pr-3 focus-within:border-[#BA9051] focus-within:ring-2 focus-within:ring-[#BA9051]/10">
        <Search className="size-[17px] shrink-0 text-[#BA9051]" strokeWidth={1.8} />
        <input type="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={contentType === "pdfs" ? "Pesquisar material PDF..." : "Pesquisar exercício..."} aria-label={contentType === "pdfs" ? "Pesquisar materiais PDF" : "Pesquisar exercícios"} className="min-w-0 flex-1 bg-transparent text-base text-[#403a35] outline-none placeholder:text-[#a79d94] sm:text-sm" />
      </label>
      {search && <button type="button" onClick={() => setSearch("")} aria-label="Limpar pesquisa" className="absolute right-4 top-1/2 flex size-7 -translate-y-1/2 items-center justify-center rounded-lg text-[#91877e] transition hover:bg-[#f2ece4] hover:text-[#A97A3C]"><X className="size-4" /></button>}
    </div>
    <AnimatePresence mode="wait" initial={false}>
...
function Summary({ icon: Icon, label, value, animateFirstEntry }: { icon: typeof Users; label: string; value: number; animateFirstEntry: boolean }) {
  return <motion.div initial={animateFirstEntry ? { opacity: 0 } : false} animate={{ opacity: 1 }} transition={{ duration: animateFirstEntry ? 1.35 : 0, ease: "easeOut" }} className="group relative overflow-hidden rounded-2xl border border-[#e5d8c7] bg-white p-4 shadow-[0_10px_26px_rgba(64,48,30,0.05)] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_16px_34px_rgba(64,48,30,0.08)] sm:rounded-[1.25rem] sm:p-5">
...
function DeleteSessionModal({ session, loading, close, confirm }: { session: PatientSession; loading: boolean; close: () => void; confirm: () => void }) {
    ...
    <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.97 }} transition={{ duration: 0.32, ease: [0.22, 1, 0.36, 1] }} className="premium-modal-panel w-full max-w-[410px] overflow-hidden rounded-[1.5rem] border border-[#e3d3bd] bg-white shadow-[0_25px_80px_rgba(64,48,30,0.24)]">
      ...
      <div className="p-6 sm:p-7">
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0">
            <h2 className="text-base font-semibold text-[#2D2823]">Excluir sessão?</h2>
            <p className="mt-1.5 text-xs leading-relaxed text-[#746C64]">Você está prestes a excluir esta sessão do histórico.</p>
            <p className="mt-3 text-xs leading-relaxed text-[#8a8178]">Essa ação não pode ser desfeita.</p>
          </div>
        </div>
        <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          <Button type="button" variant="outline" onClick={close} disabled={loading} className="h-10 rounded-xl border-[#e6d8c5] text-xs text-[#746C64]">Cancelar</Button>
