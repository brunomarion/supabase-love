        ...
        {filteredPatients.length === 0 ? <Empty text={statusFilter === "all" ? "Nenhum paciente cadastrado ainda." : statusFilter === "active" ? "Nenhum paciente ativo encontrado." : "Nenhum paciente inativo encontrado."} /> : <>
          <div className="sm:hidden divide-y divide-[#d9c8b4]">{filteredPatients.map((p) => (
            <div key={p.id} className={`grid grid-cols-[minmax(0,1fr)_auto] grid-rows-[auto_auto] gap-x-3 gap-y-2.5 px-3 py-3 ${p.sex === "female" ? "bg-[#fff1f6] hover:bg-[#ffebf2]" : p.sex === "male" ? "bg-[#eff7ff] hover:bg-[#e7f2ff]" : "bg-white hover:bg-[#fdfbf8]"} transition-colors`}>
              ...
              <div className="flex flex-col items-end justify-start text-[10px] text-[#948a81] whitespace-nowrap leading-none"><span>Data de Cadastro:</span><span className="mt-0">{formatDate(p.created_at)}</span></div>
